...
class Solver:
    @classmethod
    def solve(cls, self, operation):
        self.equation = str(
            {"C": lambda value: "",
             "🡸": lambda value: value[:-1],
             "=": lambda value: eval(value)}.get(
                operation, lambda value: (value if value != "0" else "") + operation)(self.equation)) or "0"
        return self.display_label.configure(text=self.equation)
...
class Calculator:
    ...
    def __init__(self, logic):
        ...
        self.equation = ''
        self.display_label = Label(text=self.equation,
                                   font=("Arial Bold", 42),
                                   bg=self.canvas.BG_ALL,
                                   foreground="#FFF")
        self.display_label.place(x=0, y=1)
    ...
    def create_symbol_buttons(self):
        for symbol, grid_value in self.digits.items():
            command_solve = lambda x=symbol: self.solve.solve(self, x)
...
if __name__ == "__main__":
    logics = Solver()
    print(logics.__dict__)
    calc = Calculator(logics)
    print(calc.__dict__['solve'])
    calc.run()
